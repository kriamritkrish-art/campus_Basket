      ...
      {isOpen && (
        <div className="fixed bottom-20 md:bottom-6 right-3 sm:right-6 z-50 w-[calc(100vw-24px)] sm:w-[420px] max-h-[640px] h-[86vh] bg-white rounded-3xl shadow-2xl border border-slate-200/90 flex flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-5 duration-200">
          ...
          {micPermission !== 'granted' && micPermission !== 'denied' && (
            <div className="px-3.5 py-2.5 bg-emerald-50 border-b border-emerald-200 text-emerald-900 flex items-center justify-between gap-2 shrink-0 animate-in fade-in">
              <div className="flex items-center gap-1.5 text-xs font-semibold text-emerald-800">
                <Mic className="w-4 h-4 text-[#4F9D2F] shrink-0 animate-pulse" />
                <span>Microphone access needed for voice</span>
              </div>
              <button
                type="button"
                onClick={() => requestMicPermission()}
                className="px-3 py-1.5 rounded-xl bg-[#4F9D2F] hover:bg-[#438727] text-white text-xs font-bold shadow-md transition-all active:scale-95 cursor-pointer shrink-0"
              >
                Allow Microphone
              </button>
            </div>

...
export class EmailService {
  ...
  /**
   * Sends the 6-digit registration / verification OTP.
   */
  async sendOtpEmail(recipientEmail: string, otp: string, purposeDescription?: string): Promise<boolean> {
    ...
    const html = `
      <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 540px; margin: 0 auto; padding: 24px; background: #ffffff; border: 1px solid #e2e8f0; border-radius: 12px;">
        ...
        <div style="background: #f8fafc; border: 2px dashed #689f38; border-radius: 8px; padding: 18px; text-align: center; margin: 24px 0;">
          <span style="font-size: 32px; font-weight: 700; letter-spacing: 8px; color: #2e7d32; font-family: monospace;">
            ${otp}
          </span>
        </div>
        
        <p style="color: #64748b; font-size: 13px; line-height: 1.5;">
          If you did not initiate this request on Campus Basket, please ignore this email. Never share your OTP with anyone.
        </p>
    ...
    console.info(`[EMAIL SERVICE] Purpose: ${purpose}`);

...
export default function AdminFoodServicesPage() {
  ...
  const fetchFoodData = async () => {
    setLoading(true);
    try {
      const res = await apiRequest('/api/admin/services/food');
      if (res.success) {
        setData(res);
      }
    } catch (err) {
      console.warn('Error loading food data:', err);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-[#17202A] flex items-center gap-2">
            <Utensils className="w-5 h-5 text-[#4F9D32]" />
            <span>Food &amp; Meals Administration</span>
            <span className="text-[10px] bg-[#4F9D32]/10 text-[#347A27] font-bold px-2 py-0.5 rounded-full border border-[#4F9D32]/20">
              Kitchen Operations
            </span>
          </h1>
          <p className="text-xs text-slate-500 mt-1">
            Hot meals, midnight biryani, hostel snacks, and south Indian specials management
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <AdminKpiCard
          ...
          icon={IndianRupee}
          subtitle="Lifetime verified sales"
          color="green"
        />
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden">
        <div className="p-4 sm:p-5 border-b border-slate-200 flex items-center justify-between">
          <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider">
            Active Food &amp; Meals Catalog ({products.length})
          </h3>
        </div>
        ...
        {loading ? (
          <div className="py-20 flex flex-col items-center justify-center gap-2">
            ...
            <span className="text-xs text-slate-500 font-medium">Loading food menu...</span>
          </div>
        ) : (
            ...
            <table className="w-full text-left text-xs">
              <thead className="bg-slate-50 text-[10px] uppercase font-bold text-slate-500 border-b border-slate-200">
                <tr>
                  ...
                  <th className="py-3 px-4">Estimated Prep</th>
                ...
                {products.map((p: any) => (
                  <tr key={p.id} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="text-[10px] text-slate-500 font-mono font-normal mt-0.5">
                            {p.unit} &bull; {p.provider?.businessName || 'Canteen'}
                          </div>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="py-3.5 px-4 font-mono">
                      <span className="font-bold text-sm text-[#17202A]">₹{p.discountPrice || p.price}</span>
                      {p.discountPrice && (
                    ...
                    <td className="py-3.5 px-4">
                      ...
                      >
                        {p.availability ? 'SERVING NOW' : 'OFFLINE'}
                      </span>
                    </td>
      ...
      <ProductFormModal
        ...
        categories={subcategories.length > 0 ? subcategories : [{ id: 'cat_food', name: 'Food & Meals' }]}

...
export default function ProductAnalyticsClient() {
  ...
  useEffect(() => {
    ...
    if (id && id !== 'default') loadProductAnalytics();
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="p-6 bg-white rounded-3xl border border-slate-200 shadow-xs flex flex-col md:flex-row items-center gap-6">
        ...
        <div className="space-y-1.5 flex-1">
          <div className="flex items-center gap-2">
            ...
            <span className="font-mono text-xs text-slate-400">SKU: {product.sku || product.id || 'N/A'}</span>
          </div>
          <h1 className="text-xl font-black text-[#17202A]">{product.name || 'Product Analytics'}</h1>
          <p className="text-xs text-slate-500">{product.description || 'No description provided'}</p>
        </div>

        <div className="text-right font-mono shrink-0">
          <div className="text-2xl font-black text-[#17202A]">₹{product.price || 0}</div>
          <div className="text-xs text-slate-400">Stock: {product.stock || 0} units</div>
        </div>
      </div>
      ...
      <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-xs space-y-4">
        <div>
          ...
          <p className="text-xs text-slate-500">Distribution of {product.name || 'Product'} orders per hall</p>
        </div>
